...
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
#Loop to graph GWAS hits at a threshold of 0.001
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
...
ggplot(data=gather_test_Enrichment, aes(x=pos_bin, y=-log10(p), color=inv)) + geom_line() + facet_grid(.~CHR)
...
#experiment with making graph outputs 
PhenoGather2 <- foreach(i=1:2) %do% {
  #i = 1
  setwd("/Users/dgg/Desktop/Thesis/GWAS STUFF/project/berglandlab/Yang_Adam/YangsGWAS/GWAS_withoutGRMs/Dariaphenotypes")
  data = VectorOfPheno[i] #set data
  data = as.data.table(read.delim(data)) #loads data
  ...
  data[CHR =="3R" & POS <= 29031297 & POS >= 21406917, inv:="3RMo"]
  data[CHR =="3R" & is.na(inv), inv:="outside-3RMo"]
  data2 <- data[,list(nTop = c(mean(rnp < 0.05)), thr = c(0.05), n=length(rnp)), list(CHR, PVAL, pa, rnp, pos_bin, inv)]
  data2[,p:=pbinom(nTop*n, n, thr, lower.tail=F)]
  data2[,Phenotype:= VectorOfPheno[i]] #can use a split to take off file extensions from names 
  data3 <- ggplot(as.data.frame(data2, Phentope == VectorOfPheno[i]), aes(x = pos_bin, y = -log10(pa), color = inv)) +
    geom_point(alpha = 0.5) +
    geom_line() +
    facet_grid(~CHR) +
    ggtitle(VectorOfPheno[i]) + #ggtitle(data2$Phenotype)
    theme(plot.title = element_text(hjust = 0.5))
  return(data3)
}'
