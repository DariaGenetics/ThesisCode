...
Two-HeptanoneAttraction_standard_M <- read.delim("2-HeptanoneAttraction_standard_male.nogrms.original.txt",
                                                 stringsAsFactor = FALSE)
Two-HeptanoneAttraction_standard_F <- read.delim("2-HeptanoneAttraction_standard_female.nogrms.original.txt",
                                                 stringsAsFactor = FALSE)
One-HexanolAttraction_standard_F <- read.delim("1-HexanolAttraction_standard_female.nogrms.original.txt",
                                               stringsAsFactor = FALSE) 
...
CentroidSize_Standard_M <- read.delim("CentroidSize_standard_M.nogrms.jan.txt",
                                      stringsAsFactor = FALSE)
HexanalAttraction_standard_M <- read.delim("HexanalAttraction_standard_male.nogrms.original.txt",
                                           stringsAsFactor = FALSE)
HexanalAttraction_standard_F <- read.delim("HexanalAttraction_standard_female.nogrms.original.txt",
                                           stringsAsFactor = FALSE)
IntensityOfResponse_standard_F <- read.delim("IntensityOfResponse_standard_F.nogrms.original.txt",
                                             stringsAsFactor = FALSE)
...
InternocularDistance_standard_M <- read.delim("InternocularDistance_standard_M.nogrms.jan.txt",
  ...
  PupalCaseLength_Standard_Mix <- read.delim("PupalCaseLength_Standard_Mix.nogrms.jan.txt",
                                                                  stringsAsFactor = FALSE) 
  WingCentroidSize_standard_M <- read.delim("WingCentroidSize_standard_male.nogrms.original.txt",
                                             stringsAsFactor = FALSE)
  WingCentroidSize_standard_F <- read.delim("WingCentroidSize_standard_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  Weight_PooledDiets_M <- read.delim("Weight_PooledDiets_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  Weight_LowGlucoseDiet_M <- read.delim("Weight_LowGlucoseDiet_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  Weight_HighGlucoseDiet_M <- read.delim("Weight_HighGlucoseDiet_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  TriglycerideLevels_PooledDiets_M <- read.delim("TriglycerideLevels_PooledDiets_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  TriglycerideLevels_LowGlucoseDiet_M <- read.delim("TriglycerideLevels_LowGlucoseDiet_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  ThoraxLength_standard_F <- read.delim("ThoraxLength_standard_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  SolubleProteinLevels_PooledDiets_M <- read.delim("SolubleProteinLevels_PooledDiets_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  SolubleProteinLevels_LowGlucoseDiet_M <- read.delim("SolubleProteinLevels_LowGlucoseDiet_male.nogrms.original.txt",
                stringsAsFactor = FALSE) 
  NegativeGeotaxis_MSB_Treatment_F <- read.delim("NegativeGeotaxis_MSB_Treatment_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  MeanElutionTime_RepeatedEthanolExposures_M <- read.delim("MeanElutionTime_RepeatedEthanolExposures_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  MeanElutionTime_RepeatedEthanolExposures_F <- read.delim("MeanElutionTime_RepeatedEthanolExposures_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  LifeTimeFecundity_LowYeast_F <- read.delim("LifeTimeFecundity_LowYeast_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  FreeGlycerolLevels_PooledDiets_M <- read.delim("FreeGlycerolLevels_PooledDiets_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  FreeGlycerolLevels_LowGlucoseDiet_M <- read.delim("FreeGlycerolLevels_LowGlucoseDiet_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  FreeGlycerolLevels_HighGlucoseDiet_M <- read.delim("FreeGlycerolLevels_HighGlucoseDiet_male.nogrms.original.txt",
                stringsAsFactor = FALSE) 
  FoodIntake_standard_F <- read.delim("FoodIntake_standard_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  DaySleep_standard_F <- read.delim("DaySleep_standard_Female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  DayBoutNumber_standard_F <- read.delim("DayBoutNumber_standard_Female.nogrms.original.txt",
                stringsAsFactor = FALSE)
  ChillComaRecoveryTime_standard_M <- read.delim("ChillComaRecoveryTime_standard_male.nogrms.original.txt",
                stringsAsFactor = FALSE)
  ChillComaRecoveryTime_standard_F <- read.delim("ChillComaRecoveryTime_standard_female.nogrms.original.txt",
                stringsAsFactor = FALSE)
...
#facet by phenotype class 

x <- as.data.table(CentroidSize_Standard_F)
x[,rnp:=rank(PVAL)/(length(PVAL)+1)] #translates pval into a % rank
  #x[rnp < 1e-6]
x[,pos_bin:=round(POS, -4)] #the '-4' affects which bin positions are placed in. -5 results in all in pos_bin 0; -3, broken up by hundreds POS place
x[CHR =="2L" & POS <= 13154180 & POS >= 2225744, inv:="2Lt"]
x[CHR =="2L" & is.na(inv), inv:="outside-2Lt"]
x[CHR =="3R" & POS <= 29031297 & POS >= 21406917, inv:="3RMo"]
x[CHR =="3R" & is.na(inv), inv:="outside-3RMo"]



a <- x[,list(nTop = c(mean(rnp < 0.001), mean(rnp<0.01), mean(rnp < 0.1)), thr = c(0.001, 0.01, 0.1), n=length(rnp)), 
        list(CHR,pos_bin, inv)] #add col for p value from binom test 
  #a <- x[,list(nTop = mean(rnp <0.0005), n=length(rnp)), list(CHR, inv)]
a[,p:=pbinom(nTop*n, n, thr, lower.tail=F)]
a[,pa:=p.adjust(p)]
'SHOULD ADJUST BEFORE REMOVING MAJORITY OF DATASET'
  #ex <- binom.test(a$nTop[1] * a$n[1], a$n[1], 0.5) #test to see if enrichment is chromosome specific 
Chr2Land3R <- subset(a, CHR == "2L" | CHR == "3R")
Chr2Land3R <- subset(Chr2Land3R, thr == 0.001)

######graph variations#####
EnrichmentTestPlotNonadjustedP <- ggplot(a, aes(x = pos_bin, y = -log10(p), color = inv)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_grid(thr ~ CHR)
  #all chromosomes, thr of 0.001, 0.01, 0.1, p not adjusted

EnrichmentTestPlotAdjustedP <- ggplot(a, aes(x = pos_bin, y = -log10(pa), color = inv)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_grid(thr ~ CHR)
  #all chromosomes, thr of 0.001, 0.01, 0.1, p adjusted

EnrichmentTestPlot2Land3R_oneThr <- ggplot(Chr2Land3R, aes(x = pos_bin, y = -log10(pa), color = inv)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_grid(~CHR) +
  ggtitle('adjusted p-values for threshold 0.001 confidence') + 
  theme(plot.title = element_text(hjust = 0.5))
#CHR 2L, 3R, thr 0.001, p adjusted
